...
import type { SessionCard, SessionListResponse, Verdict } from "@/types";
...
export default function SessionsPage() {
  ...
  useEffect(() => {
    ...
    apiFetch("/api/sessions", accessToken)
      .then((res) => {
  ...
  if (loading || authLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[var(--bg-primary)]">
        <p className="text-2xl font-[family:var(--font-display)] text-[var(--text-secondary)] italic">
          Loading sessions…
        </p>
      </div>
    );
              ...
              {sessions.map((card, index) => {
                ...
                return (
                  <motion.button
                    ...
                    onClick={() => router.push(`/summary/${card.id}`)}
