...
export function SplashHero({ onBeginClick }: SplashHeroProps) {
  ...
  const newCard = (verdictIdx: number): Card => ({
    verdictIdx,
    phase: "verifying",
    key: ++cardKeyRef.current,
  });

  const [cards, setCards] = useState<Card[]>([newCard(0), newCard(1), newCard(2), newCard(3)]);
          ...
          setTimeout(() => {
            ...
            timeouts.push(
              setTimeout(() => {
                if (cancelled) return;
                setCards((prev) =>
                  prev.map((c) => (c.key === card.key ? { ...c, phase: "resolved" } : c)),
                );
              }, 1800),
            );
          }, i * 800),
  ...
  return (
    ...
    >
      <div className="flex w-full max-w-[900px] flex-col items-start space-y-12">
                  ...
                  >
                    ...
                    <motion.div
                      animate={{
                        backgroundColor:
                          card.phase === "resolved"
                            ? VERDICTS[card.verdictIdx].color
                            : "var(--border-active)",
                      }}
                    ...
                    {/* bottom-left: verifying label — absolutely anchored so it never affects badge layout */}
                    <div style={{ position: "absolute", bottom: "14px", left: "22px" }}>
                      <AnimatePresence mode="wait">
                        {card.phase === "verifying" && (
                          <motion.div
                            key="v"
                    ...
                    {/* bottom-right: verdict badge — absolutely anchored to right edge */}
                      ...
                      <AnimatePresence mode="wait">
                        {card.phase === "resolved" && (
                          <motion.div
                            ...
                            style={{
                              color: VERDICTS[card.verdictIdx].color,
                              fontSize: "0.6rem",
        ...
        {/* Steps Grid */}
        <div className="grid w-full grid-cols-1 gap-12 pt-4 md:grid-cols-3">
          {steps.map((step, i) => (
            <motion.div
              key={step.num}
              variants={fadeUpVariants}
            ...
            >
              ...
              <h3 className="text-xl font-[family:var(--font-display)] font-bold tracking-wide text-[var(--text-primary)]">
                {step.title}
              </h3>
              <p className="text-base leading-relaxed font-[family:var(--font-body)] text-[var(--text-secondary)]">
                {step.desc}
              </p>
            </motion.div>
          ))}
      ...
      <div className="pointer-events-none absolute top-0 right-0 h-full w-1/3 bg-gradient-to-l from-[var(--bg-card)] to-transparent opacity-20" />
    </motion.section>
  );
}
